Ext.grid.RowEditor = Ext.extend(Ext.Panel, {
    ...
    startEditing: function(rowIndex, doFocus){
        ...
        if(this.fireEvent('beforeedit', this, rowIndex) !== false){
            ...
            var cm = g.getColumnModel(), fields = this.items.items, f, val, j = 0;
            for(var i = 0, len = cm.getColumnCount(); i < len; i++){
            	if (cm.isLocked && cm.isLocked(i)) {
            		continue;
            	}
    ...
    stopEditing : function(saveChanges){
        ...
        var changes = {},
            ...
            j = 0;
        for(var i = 0, len = cm.getColumnCount(); i < len; i++){
            if(!cm.isHidden(i) && (cm.isLocked && !cm.isLocked(i))){
                var dindex = cm.getDataIndex(i);
                if(!Ext.isEmpty(dindex)){
                    var oldValue = r.data[dindex],
                        value = this.postEditValue(fields[j].getValue(), oldValue, r, dindex);
                    j++;
    ...
    verifyLayout: function(force){
        if(this.el && (this.isVisible() || force === true)){
            ...
            var cm = this.grid.colModel, fields = this.items.items, j = 0;
            for(var i = 0, len = cm.getColumnCount(); i < len; i++){
            	if (cm.isLocked && cm.isLocked(i)) {
            		continue;
            	}
                if(!cm.isHidden(i)){
                    ...
                    fields[j].show();
                    fields[j].setWidth(cm.getColumnWidth(i) - adjust);
                } else{
                    fields[j].hide();
                }
                j++;
    ...
    initFields: function(){
        ...
        for(var i = 0, len = cm.getColumnCount(); i < len; i++){
            ...
            var c = cm.getColumnAt(i);
                ed = (c.getEditor ? c.getEditor() : undefined);
            if(!ed){
...
Ext.preg('roweditor', Ext.grid.RowEditor);
